...
#include "pch.h"
#include "BasicTessIsoline.hpp"


using namespace std;


// Example data for basic isoline
...
// Simple 2 vertex isoline patch (no adjacency)
...
BasicTessIsoline::BasicTessIsoline() {
	...
	shaderProg = GLShaderFactory::CreateProgramObject(
		...
		GLShaderFactory::CreateShader(GL_TESS_CONTROL_SHADER, "shaders\\Tessellation\\basic_tess_iso.tcs.txt"),
		GLShaderFactory::CreateShader(GL_TESS_EVALUATION_SHADER, "shaders\\Tessellation\\basic_tess_iso.tes.txt"),
...
void BasicTessIsoline::render(const glm::mat4& T) {
	...
	glPatchParameteri(GL_PATCH_VERTICES, 2); // 2 vertices per patch
	glDrawElements(GL_PATCHES, 2, GL_UNSIGNED_INT, (const GLvoid*)0);
}
