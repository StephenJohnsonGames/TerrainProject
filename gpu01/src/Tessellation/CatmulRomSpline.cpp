...
CatmulRomSpline::CatmulRomSpline() {
	...
	shaderProg = GLShaderFactory::CreateProgramObject(
		...
		GLShaderFactory::CreateShader(GL_TESS_CONTROL_SHADER, "shaders\\Tessellation\\catmul_rom.tcs.txt"),
		GLShaderFactory::CreateShader(GL_TESS_EVALUATION_SHADER, "shaders\\Tessellation\\catmul_rom.tes.txt"),
...
void CatmulRomSpline::render(const glm::mat4& T) {
	...
	glDrawElements(GL_PATCHES, 12, GL_UNSIGNED_INT, (const GLvoid*)0);
}
